...
def calculateBoxCheckSum(input):
    ...
    for box in input:
        ...
        for character in box:
            if character in boxCounts:
                boxCounts[character] += 1
            else:
                boxCounts[character] = 1

        # Only need to count these once
        gotCheckSumTwo = False
        gotCheckSumThree = False

        for key, value in boxCounts.items():
            if (gotCheckSumTwo and gotCheckSumThree):
                break
            if value == 2 and not gotCheckSumTwo:
                checkSumTwo += 1
                gotCheckSumTwo = True
            elif value == 3 and not gotCheckSumThree:
                checkSumThree +=1
                gotCheckSumThree = True
            else:
                continue

    print(checkSumTwo * checkSumThree)

#calculateBoxCheckSum(testInput)
#calculateBoxCheckSum(inputList)

testInputPrototype = ["abcde", "fghij", "klmno", "pqrst", "fguij", "axcye", "wvxyz"]

def determinePrototypeBox(input):
    breakLoop = False
    for boxIndex in range(0, len(input[0])):
        if breakLoop:
            break
        ...
        for box in input:
            processedBox = box[:boxIndex] + box[boxIndex + 1:]

            if processedBox in boxStringSet:
                print processedBox
                breakLoop = True
                break
            else:
                boxStringSet.add(processedBox)

determinePrototypeBox(testInputPrototype)
determinePrototypeBox(inputList)